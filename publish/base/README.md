...
The working repository has its own project-specific `.cortex/` state. External users should use the clean generated base under `publish/base/`.

Regenerate it with:

```powershell
.\tools\Build-PublishBase.ps1 -Clean
```

`publish/base/` excludes the working repo's `.cortex/` folder and includes the blank template, docs, tools, schemas, prompts, and community files needed by other projects.
...
`schemas/` contains JSON Schemas for `.cortex/meta.json` and `.cortex/mcp-tools.json`. These are intended for future MCP servers, n8n workflows, validation scripts, and external agents.
...
Codex Cortex is intentionally split into human-readable files, machine-readable JSON, prompt packs, local PowerShell utilities, and a generated `publish/base/` folder for project-free distribution.

For integration guidance, read `docs/INTEGRATIONS.txt`.
